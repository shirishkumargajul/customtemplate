...
public class MacroOperatorDao {
	...
	/*private MacroOperatorQuery macro;
	...
	public String getDataFromARI(int apprid) {
		...
		// tempPreconditionFeaturesList.get(i).get(1));
		// }
		return Flag;
	}
	...
	public void insertMacroOperator2Database(MacroOperatorQuery mo) {
		...
		System.out.println(macro.getApplicationIdentifie() + " || " + macro.getDomain() + " || " + macro.getResource()
				+ " || " + macro.getPrecondition());
		Connection connection = MysqlJdbc.getConnection();
	...
	public MacroOperatorQuery getMacro() {
		return macro;
	}

	public void setMacro(MacroOperatorQuery macro) {
		this.macro = macro;
	}
	...
	public void setPreconditionFeaturesList(List<List<String>> preconditionFeaturesList) {
	...
	public Collection<MacroOperator> getMacOps(MacroOperatorQuery query) {
		...
		try {
			...
			String sql = "select * from Demo_schema.RSpace_VMs where ram_units in('" + query.getRamUnitsLB() + ",'" + query.getRamUnitsUB() + "') "
					+ "and storage_disk = '" + query.getStorageDisk() + "' and" 
					+ "os_arch = '" + query.getOsArch() + "' and ram_size > " + 4 + " and ram_size < " + 10 ;
			stmt = connection.createStatement();
			ResultSet rs = stmt.executeQuery(sql);
	...
	public List<String> insertMacOpsToDB(Collection<MacroOperator> moCollection, String ariId) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery(sql);
			while (rs.next()) {
				macIdList.add(rs.getString("ari_id"));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
